...
export async function sendMatchPropertiesEmail(project,properties) {
let commercial;
  const client = await Client.findById(project.clientId).lean();
  ...
  sendEmailWithTemplateMatching({
    ...
    email: client.email,
    name: client.displayName,
    cc:commercial ? commercial.email :"contact@vision-r.re",
    bien_immos:properties.map((p)=> ({
      img :p.photos[0],
      name:p.name,
      price :p.salesPrice,
      lien_site:p.url_matching,
